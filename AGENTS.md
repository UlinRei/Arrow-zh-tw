...
- UID 是底層資源識別值，必須維持唯一且不可重用；刪除資源時不得回退或重設 UID seed。
- 新增場景、巨集、變數與角色時，預設名稱使用各自類型中最小的可用十進位序號。
- 新增節點時，預設名稱使用所屬場景或巨集內最小的可用十進位序號。
- 預設名稱不得直接使用 UID 或 UID 的 base36 表示；既有資源不得因命名規則更新而自動改名。
